<script>
  export default {
    name: 'md-no-encryption',

    functional: true,

    // serverCacheKey: props => `md-no-encryption:${props.width}:${props.height}:${props.className || 'icon'}`,
    ...
    render (h, renderContext) {
      ...
      return h('svg', {
        staticClass: `icon md-icon md-no-encryption${staticClass ? ' ' + staticClass : ''}`,
        'class': className,
        attrs: {
          width: width,
          height: height,
