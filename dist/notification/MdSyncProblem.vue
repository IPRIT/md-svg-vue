<script>
  export default {
    name: 'md-sync-problem',

    functional: true,

    // serverCacheKey: props => `md-sync-problem:${props.width}:${props.height}:${props.className || 'icon'}`,
    ...
    render (h, renderContext) {
      ...
      return h('svg', {
        staticClass: `icon md-icon md-sync-problem${staticClass ? ' ' + staticClass : ''}`,
        'class': className,
        attrs: {
          width: width,
          height: height,
