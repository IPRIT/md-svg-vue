<script>
  export default {
    name: 'md-sync-disabled',

    functional: true,

    // serverCacheKey: props => `md-sync-disabled:${props.width}:${props.height}:${props.className || 'icon'}`,
    ...
    render (h, renderContext) {
      ...
      return h('svg', {
        staticClass: `icon md-icon md-sync-disabled${staticClass ? ' ' + staticClass : ''}`,
        'class': className,
        attrs: {
          width: width,
          height: height,
