<script>
  export default {
    name: 'md-folder-special',

    functional: true,

    // serverCacheKey: props => `md-folder-special:${props.width}:${props.height}:${props.className || 'icon'}`,
    ...
    render (h, renderContext) {
      ...
      return h('svg', {
        staticClass: `icon md-icon md-folder-special${staticClass ? ' ' + staticClass : ''}`,
        'class': className,
        attrs: {
          width: width,
          height: height,
