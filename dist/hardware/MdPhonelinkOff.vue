<script>
  export default {
    name: 'md-phonelink-off',

    functional: true,

    // serverCacheKey: props => `md-phonelink-off:${props.width}:${props.height}:${props.className || 'icon'}`,
    ...
    render (h, renderContext) {
      ...
      return h('svg', {
        staticClass: `icon md-icon md-phonelink-off${staticClass ? ' ' + staticClass : ''}`,
        'class': className,
        attrs: {
          width: width,
          height: height,
