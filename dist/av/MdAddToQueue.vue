<script>
  export default {
    name: 'md-add-to-queue',

    functional: true,

    // serverCacheKey: props => `md-add-to-queue:${props.width}:${props.height}:${props.className || 'icon'}`,
    ...
    render (h, renderContext) {
      ...
      return h('svg', {
        staticClass: `icon md-icon md-add-to-queue${staticClass ? ' ' + staticClass : ''}`,
        'class': className,
        attrs: {
          width: width,
          height: height,
