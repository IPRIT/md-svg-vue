<script>
  export default {
    name: 'md-format-clear',

    functional: true,

    // serverCacheKey: props => `md-format-clear:${props.width}:${props.height}:${props.className || 'icon'}`,
    ...
    render (h, renderContext) {
      ...
      return h('svg', {
        staticClass: `icon md-icon md-format-clear${staticClass ? ' ' + staticClass : ''}`,
        'class': className,
        attrs: {
          width: width,
          height: height,
