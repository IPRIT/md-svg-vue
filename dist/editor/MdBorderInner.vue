<script>
  export default {
    name: 'md-border-inner',

    functional: true,

    // serverCacheKey: props => `md-border-inner:${props.width}:${props.height}:${props.className || 'icon'}`,
    ...
    render (h, renderContext) {
      ...
      return h('svg', {
        staticClass: `icon md-icon md-border-inner${staticClass ? ' ' + staticClass : ''}`,
        'class': className,
        attrs: {
          width: width,
          height: height,
