<script>
  export default {
    name: 'md-format-shapes',

    functional: true,

    // serverCacheKey: props => `md-format-shapes:${props.width}:${props.height}:${props.className || 'icon'}`,
    ...
    render (h, renderContext) {
      ...
      return h('svg', {
        staticClass: `icon md-icon md-format-shapes${staticClass ? ' ' + staticClass : ''}`,
        'class': className,
        attrs: {
          width: width,
          height: height,
