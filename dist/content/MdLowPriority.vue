<script>
  export default {
    name: 'md-low-priority',

    functional: true,

    // serverCacheKey: props => `md-low-priority:${props.width}:${props.height}:${props.className || 'icon'}`,
    ...
    render (h, renderContext) {
      ...
      return h('svg', {
        staticClass: `icon md-icon md-low-priority${staticClass ? ' ' + staticClass : ''}`,
        'class': className,
        attrs: {
          width: width,
          height: height,
