<script>
  export default {
    name: 'md-local-activity',

    functional: true,

    // serverCacheKey: props => `md-local-activity:${props.width}:${props.height}:${props.className || 'icon'}`,
    ...
    render (h, renderContext) {
      ...
      return h('svg', {
        staticClass: `icon md-icon md-local-activity${staticClass ? ' ' + staticClass : ''}`,
        'class': className,
        attrs: {
          width: width,
          height: height,
