<script>
  export default {
    name: 'md-local-dining',

    functional: true,

    // serverCacheKey: props => `md-local-dining:${props.width}:${props.height}:${props.className || 'icon'}`,
    ...
    render (h, renderContext) {
      ...
      return h('svg', {
        staticClass: `icon md-icon md-local-dining${staticClass ? ' ' + staticClass : ''}`,
        'class': className,
        attrs: {
          width: width,
          height: height,
