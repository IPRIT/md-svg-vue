<script>
  export default {
    name: 'md-battery-unknown',

    functional: true,

    // serverCacheKey: props => `md-battery-unknown:${props.width}:${props.height}:${props.className || 'icon'}`,
    ...
    render (h, renderContext) {
      ...
      return h('svg', {
        staticClass: `icon md-icon md-battery-unknown${staticClass ? ' ' + staticClass : ''}`,
        'class': className,
        attrs: {
          width: width,
          height: height,
