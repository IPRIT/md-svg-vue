<script>
  export default {
    ...
    // serverCacheKey: props => `md-signal-wifi-1-bar-lock:${props.width}:${props.height}:${props.className || 'icon'}`,
    ...
    render (h, renderContext) {
      ...
      return h('svg', {
        staticClass: `icon md-icon md-signal-wifi-1-bar-lock${staticClass ? ' ' + staticClass : ''}`,
        'class': className,
        attrs: {
          width: width,
          height: height,
