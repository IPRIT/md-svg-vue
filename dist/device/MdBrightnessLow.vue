<script>
  export default {
    name: 'md-brightness-low',

    functional: true,

    // serverCacheKey: props => `md-brightness-low:${props.width}:${props.height}:${props.className || 'icon'}`,
    ...
    render (h, renderContext) {
      ...
      return h('svg', {
        staticClass: `icon md-icon md-brightness-low${staticClass ? ' ' + staticClass : ''}`,
        'class': className,
        attrs: {
          width: width,
          height: height,
