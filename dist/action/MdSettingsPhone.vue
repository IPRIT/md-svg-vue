<script>
  export default {
    name: 'md-settings-phone',

    functional: true,

    // serverCacheKey: props => `md-settings-phone:${props.width}:${props.height}:${props.className || 'icon'}`,
    ...
    render (h, renderContext) {
      ...
      return h('svg', {
        staticClass: `icon md-icon md-settings-phone${staticClass ? ' ' + staticClass : ''}`,
        'class': className,
        attrs: {
          width: width,
          height: height,
