<script>
  export default {
    name: 'md-settings-remote',

    functional: true,

    // serverCacheKey: props => `md-settings-remote:${props.width}:${props.height}:${props.className || 'icon'}`,
    ...
    render (h, renderContext) {
      ...
      return h('svg', {
        staticClass: `icon md-icon md-settings-remote${staticClass ? ' ' + staticClass : ''}`,
        'class': className,
        attrs: {
          width: width,
          height: height,
