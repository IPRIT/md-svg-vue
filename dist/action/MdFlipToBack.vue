<script>
  export default {
    name: 'md-flip-to-back',

    functional: true,

    // serverCacheKey: props => `md-flip-to-back:${props.width}:${props.height}:${props.className || 'icon'}`,
    ...
    render (h, renderContext) {
      ...
      return h('svg', {
        staticClass: `icon md-icon md-flip-to-back${staticClass ? ' ' + staticClass : ''}`,
        'class': className,
        attrs: {
          width: width,
          height: height,
