<script>
  export default {
    name: 'md-settings-voice',

    functional: true,

    // serverCacheKey: props => `md-settings-voice:${props.width}:${props.height}:${props.className || 'icon'}`,
    ...
    render (h, renderContext) {
      ...
      return h('svg', {
        staticClass: `icon md-icon md-settings-voice${staticClass ? ' ' + staticClass : ''}`,
        'class': className,
        attrs: {
          width: width,
          height: height,
