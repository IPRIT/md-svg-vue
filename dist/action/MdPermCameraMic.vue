<script>
  export default {
    name: 'md-perm-camera-mic',

    functional: true,

    // serverCacheKey: props => `md-perm-camera-mic:${props.width}:${props.height}:${props.className || 'icon'}`,
    ...
    render (h, renderContext) {
      ...
      return h('svg', {
        staticClass: `icon md-icon md-perm-camera-mic${staticClass ? ' ' + staticClass : ''}`,
        'class': className,
        attrs: {
          width: width,
          height: height,
