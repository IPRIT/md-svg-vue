<script>
  export default {
    name: 'md-settings-overscan',

    functional: true,

    // serverCacheKey: props => `md-settings-overscan:${props.width}:${props.height}:${props.className || 'icon'}`,
    ...
    render (h, renderContext) {
      ...
      return h('svg', {
        staticClass: `icon md-icon md-settings-overscan${staticClass ? ' ' + staticClass : ''}`,
        'class': className,
        attrs: {
          width: width,
          height: height,
