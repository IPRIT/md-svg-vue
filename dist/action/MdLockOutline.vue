<script>
  export default {
    name: 'md-lock-outline',

    functional: true,

    // serverCacheKey: props => `md-lock-outline:${props.width}:${props.height}:${props.className || 'icon'}`,
    ...
    render (h, renderContext) {
      ...
      return h('svg', {
        staticClass: `icon md-icon md-lock-outline${staticClass ? ' ' + staticClass : ''}`,
        'class': className,
        attrs: {
          width: width,
          height: height,
