<script>
  export default {
    name: 'md-exposure-neg-2',

    functional: true,

    // serverCacheKey: props => `md-exposure-neg-2:${props.width}:${props.height}:${props.className || 'icon'}`,
    ...
    render (h, renderContext) {
      ...
      return h('svg', {
        staticClass: `icon md-icon md-exposure-neg-2${staticClass ? ' ' + staticClass : ''}`,
        'class': className,
        attrs: {
          width: width,
          height: height,
