<script>
  export default {
    name: 'md-filter-9-plus',

    functional: true,

    // serverCacheKey: props => `md-filter-9-plus:${props.width}:${props.height}:${props.className || 'icon'}`,
    ...
    render (h, renderContext) {
      ...
      return h('svg', {
        staticClass: `icon md-icon md-filter-9-plus${staticClass ? ' ' + staticClass : ''}`,
        'class': className,
        attrs: {
          width: width,
          height: height,
