<script>
  export default {
    name: 'md-filter-frames',

    functional: true,

    // serverCacheKey: props => `md-filter-frames:${props.width}:${props.height}:${props.className || 'icon'}`,
    ...
    render (h, renderContext) {
      ...
      return h('svg', {
        staticClass: `icon md-icon md-filter-frames${staticClass ? ' ' + staticClass : ''}`,
        'class': className,
        attrs: {
          width: width,
          height: height,
