<script>
  export default {
    name: 'md-camera-front',

    functional: true,

    // serverCacheKey: props => `md-camera-front:${props.width}:${props.height}:${props.className || 'icon'}`,
    ...
    render (h, renderContext) {
      ...
      return h('svg', {
        staticClass: `icon md-icon md-camera-front${staticClass ? ' ' + staticClass : ''}`,
        'class': className,
        attrs: {
          width: width,
          height: height,
