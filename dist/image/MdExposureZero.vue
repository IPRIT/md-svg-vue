<script>
  export default {
    name: 'md-exposure-zero',

    functional: true,

    // serverCacheKey: props => `md-exposure-zero:${props.width}:${props.height}:${props.className || 'icon'}`,
    ...
    render (h, renderContext) {
      ...
      return h('svg', {
        staticClass: `icon md-icon md-exposure-zero${staticClass ? ' ' + staticClass : ''}`,
        'class': className,
        attrs: {
          width: width,
          height: height,
