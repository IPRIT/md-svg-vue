<script>
  export default {
    name: 'md-photo-filter',

    functional: true,

    // serverCacheKey: props => `md-photo-filter:${props.width}:${props.height}:${props.className || 'icon'}`,
    ...
    render (h, renderContext) {
      ...
      return h('svg', {
        staticClass: `icon md-icon md-photo-filter${staticClass ? ' ' + staticClass : ''}`,
        'class': className,
        attrs: {
          width: width,
          height: height,
