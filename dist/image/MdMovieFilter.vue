<script>
  export default {
    name: 'md-movie-filter',

    functional: true,

    // serverCacheKey: props => `md-movie-filter:${props.width}:${props.height}:${props.className || 'icon'}`,
    ...
    render (h, renderContext) {
      ...
      return h('svg', {
        staticClass: `icon md-icon md-movie-filter${staticClass ? ' ' + staticClass : ''}`,
        'class': className,
        attrs: {
          width: width,
          height: height,
