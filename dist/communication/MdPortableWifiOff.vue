<script>
  export default {
    name: 'md-portable-wifi-off',

    functional: true,

    // serverCacheKey: props => `md-portable-wifi-off:${props.width}:${props.height}:${props.className || 'icon'}`,
    ...
    render (h, renderContext) {
      ...
      return h('svg', {
        staticClass: `icon md-icon md-portable-wifi-off${staticClass ? ' ' + staticClass : ''}`,
        'class': className,
        attrs: {
          width: width,
          height: height,
