<script>
  export default {
    name: 'md-phonelink-lock',

    functional: true,

    // serverCacheKey: props => `md-phonelink-lock:${props.width}:${props.height}:${props.className || 'icon'}`,
    ...
    render (h, renderContext) {
      ...
      return h('svg', {
        staticClass: `icon md-icon md-phonelink-lock${staticClass ? ' ' + staticClass : ''}`,
        'class': className,
        attrs: {
          width: width,
          height: height,
